...
`AutomaticCodeModeEngine` is the default. On macOS it uses `ProcessCodeModeEngine` only for an executable selected by `SWIFT_CODEX_CODE_MODE_HOST`, bundle adjacency, or `argv[0]` adjacency. A host may instead inject an explicit executable URL. Discovery never searches the current working directory or `PATH`; unbundled non-GUI macOS processes retain the `JavaScriptCoreCodeModeEngine` fallback. On iOS, automatic execution uses `WebKitCodeModeEngine` rather than silently dropping to an in-process context.
...
The iOS host keeps trusted relay code in a hidden nonpersistent `WKWebView`, disables page scripts, and uses a private `WKContentWorld` plus `WKScriptMessageHandlerWithReply` for native RPC. Each cell's runtime and model-authored source run inside a dedicated Blob Web Worker. The worker bridge is guarded by a per-cell capability token, while CSP and worker-global lockdown (including native prototype descriptors) prevent direct network or native-bridge access. The relay canonicalizes and byte-checks every event before it crosses into Swift; decoding and output accounting then run off the main actor in message order. Host registrations are generation-scoped so a failed WebContent process cannot sweep newly queued cells into the failed generation. Completion, output, notifications, and nested tool results retain the same `CodeModeCellSession` contract as the process and JavaScriptCore engines. Termination calls `Worker.terminate()`, so a synchronous loop cannot hold the app or relay event loop. WebKit runs outside the app process but may pool cells into a shared WebContent process; this is a terminable execution boundary, not a guaranteed process-per-cell sandbox. Content-process termination fails active cells without replaying native tool calls and the next cell creates a fresh relay. The explicit in-process engine has the same code-mode surface but cannot hard-interrupt synchronous execution.
