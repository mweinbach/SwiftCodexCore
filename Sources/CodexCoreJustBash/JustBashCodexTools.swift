...
public enum JustBashCodexFactory {
  public static func makeEnvironment(
    ...
    if !embeddedSkills.isEmpty {
      let skillsRoot =
        embeddedSkillsRootURL
        ?? CodexDefaultLocations.embeddedSkillsDirectory.appendingPathComponent(
          embeddedSkillsRootName, isDirectory: true)
    ...
    parameters: ToolSchemas.object(
      properties: [
        "command": ToolSchemas.string(
          description: "Command to execute with the embedded JustBash interpreter"),
...
public struct JustBashFileReadTool: AgentTool {
    ...
    parameters: ToolSchemas.object(
      properties: [
        "path": ToolSchemas.string(
          description: "Virtual path relative to the JustBash working directory")
      ], required: ["path"])
  )
  ...
  public func run(arguments: JSONValue, context: ToolExecutionContext) async throws -> ToolResult {
    ...
    return ToolResult(
      content: result.stdout, metadata: ["path": .string(path), "runtime": .string("justbash")])
  }
}
...
public struct JustBashFileWriteTool: AgentTool {
  ...
  public func run(arguments: JSONValue, context: ToolExecutionContext) async throws -> ToolResult {
    ...
    return ToolResult(
      content: "Wrote \(content.utf8.count) bytes to \(path)",
      metadata: ["path": .string(path), "runtime": .string("justbash")])
  }
}
...
public struct JustBashEditFileTool: AgentTool {
  ...
  public func run(arguments: JSONValue, context: ToolExecutionContext) async throws -> ToolResult {
    ...
    guard occurrences > 0 else {
      throw CodexCoreError.invalidState("Old text not found in \(path)")
    }
    ...
    return ToolResult(
      content: "Edited \(path): replaced \(replaceAll ? occurrences : 1) occurrence(s)",
      metadata: ["runtime": .string("justbash")])
  }
}
...
public struct JustBashApplyPatchTool: AgentTool {
    ...
    parameters: ToolSchemas.object(
      properties: [
        ...
        "strip": .object([
          ...
          "minimum": .number(0),
          "description": .string(
            "Additional leading path components to strip after a/ and b/ prefixes are removed"),
        ]),
      ], required: ["patch"]),
    requiresApproval: true,
    isStateChanging: true
  ...
  public func run(arguments: JSONValue, context: ToolExecutionContext) async throws -> ToolResult {
    ...
    let changed = try await UnifiedDiffApplier.apply(
      patch: patch, strip: strip, bash: bash, cwd: defaultCWD)
    return ToolResult(
      content: "Applied patch to \(changed.joined(separator: ", "))",
      structuredContent: .array(changed.map(JSONValue.string)),
      metadata: ["runtime": .string("justbash")])
  }
}
...
private func writeVirtualFile(path: String, content: String, bash: Bash, cwd: String?) async throws
{
  if let parent = virtualParent(of: path), parent != "." {
    ...
    if mkdir.exitCode != 0 {
      throw CodexCoreError.invalidState(mkdir.stderr.isEmpty ? mkdir.stdout : mkdir.stderr)
    }
  }
  let result = await bash.exec(
    "cat > \(shellQuote(path))", options: ExecOptions(cwd: cwd, stdin: content))
  if result.exitCode != 0 {
    throw CodexCoreError.invalidState(result.stderr.isEmpty ? result.stdout : result.stderr)
  }
}
...
private enum UnifiedDiffApplier {
  ...
  static func apply(patch: String, strip: Int, bash: Bash, cwd: String?) async throws -> [String] {
    ...
    var changed: [String] = []
    for file in files {
      let path = normalizePatchPath(
        file.newPath == "/dev/null" ? file.oldPath : file.newPath, strip: strip)
  ...
  private static func parse(_ patch: String) throws -> [FilePatch] {
    ...
    while index < lines.count {
      ...
      let oldPath =
        String(lines[index].dropFirst(4)).split(separator: "\t", maxSplits: 1).first.map(
          String.init) ?? ""
      index += 1
