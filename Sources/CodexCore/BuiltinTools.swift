...
public struct FileReadTool: AgentTool {
  public let definition = ToolDefinition(
    ...
    parameters: ToolSchemas.object(
      properties: [
        "path": ToolSchemas.string(
          description: "File path relative to the workspace, or absolute path if permitted")
      ], required: ["path"])
  )
  ...
  public func run(arguments: JSONValue, context: ToolExecutionContext) async throws -> ToolResult {
    ...
    let url = try resolvePath(
      try arguments.requiredString("path"), context: context, forWrite: false)
...
public struct FileWriteTool: AgentTool {
  public let definition = ToolDefinition(
    name: "write_file",
    description:
      "Write a UTF-8 text file inside the workspace. Creates parent directories when needed.",
    parameters: ToolSchemas.object(
      properties: [
  ...
  public func run(arguments: JSONValue, context: ToolExecutionContext) async throws -> ToolResult {
    ...
    let url = try resolvePath(
      try arguments.requiredString("path"), context: context, forWrite: true)
    try FileManager.default.createDirectory(
      at: url.deletingLastPathComponent(), withIntermediateDirectories: true)
    let content = try arguments.requiredString("content")
    try content.data(using: .utf8)?.write(to: url, options: [.atomic])
    return ToolResult(
      content: "Wrote \(content.utf8.count) bytes to \(url.path)",
...
public struct ListFilesTool: AgentTool {
  ...
  public func run(arguments: JSONValue, context: ToolExecutionContext) async throws -> ToolResult {
    ...
    if recursive {
      urls =
        (FileManager.default.enumerator(at: url, includingPropertiesForKeys: [.isDirectoryKey])?
          .compactMap { $0 as? URL } ?? [])
    } else {
...
public struct EditFileTool: AgentTool {
  public let definition = ToolDefinition(
    name: "edit_file",
    description:
      "Replace an exact string in a UTF-8 file. Fails if the old text is missing or appears more than once unless replace_all is true.",
    parameters: ToolSchemas.object(
      properties: [
        "path": ToolSchemas.string(description: "File path relative to the workspace"),
        "old": ToolSchemas.string(description: "Exact old text"),
        "new": ToolSchemas.string(description: "Replacement text"),
        "replace_all": ToolSchemas.boolean(
          description: "Replace all occurrences instead of requiring a single match"),
  ...
  public func run(arguments: JSONValue, context: ToolExecutionContext) async throws -> ToolResult {
    ...
    guard occurrences > 0 else {
      throw CodexCoreError.invalidState("Old text not found in \(url.path)")
    }
    if !replaceAll && occurrences != 1 {
      throw CodexCoreError.invalidState(
        "Old text occurs \(occurrences) times. Set replace_all=true or use more context.")
    }
    text = text.replacingOccurrences(of: old, with: new)
    try text.data(using: .utf8)?.write(to: url, options: [.atomic])
    return ToolResult(
      content: "Edited \(url.path): replaced \(replaceAll ? occurrences : 1) occurrence(s)")
  }
}
  ...
  public struct ShellTool: AgentTool {
    public let definition = ToolDefinition(
      name: "shell",
      description:
        "Run a shell command in the workspace and return combined stdout/stderr. Use only when needed.",
      parameters: ToolSchemas.object(
        properties: [
          ...
          "timeout_seconds": .object([
            "type": .string("number"), "description": .string("Optional timeout in seconds"),
          ]),
        ], required: ["command"]),
      requiresApproval: true,
      isStateChanging: true
    ...
    public func run(arguments: JSONValue, context: ToolExecutionContext) async throws -> ToolResult
    {
      ...
      let cwd = try resolvePath(
        arguments.optionalString("cwd") ?? ".", context: context, forWrite: false)
      let timeout = TimeInterval(arguments["timeout_seconds"]?.doubleValue ?? 120)
      let result = try await ProcessRunner.run(
        "/bin/sh", arguments: ["-lc", command], currentDirectory: cwd, stdin: nil, timeout: timeout)
  ...
  public struct ApplyPatchTool: AgentTool {
      ...
      parameters: ToolSchemas.object(
        properties: [
          ...
          "strip": .object([
            "type": .string("integer"),
            "minimum": .number(0),
            "description": .string("patch -p strip count, defaults to 0"),
          ]),
        ], required: ["patch"]),
      requiresApproval: true,
      isStateChanging: true
    ...
    public func run(arguments: JSONValue, context: ToolExecutionContext) async throws -> ToolResult
    {
      ...
      let workspace =
        context.workspaceURL ?? URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
      let patch = try arguments.requiredString("patch")
      let strip = try patchStripCount(arguments["strip"])
      let result = try await ProcessRunner.run(
        "/usr/bin/patch", arguments: ["--batch", "--forward", "-p\(strip)"],
        currentDirectory: workspace, stdin: patch, timeout: 120)
      let output = [result.stdout, result.stderr].filter { !$0.isEmpty }.joined(separator: "\n")
      return ToolResult(
        content: output.isEmpty ? "patch exited with status \(result.exitCode)" : output,
        isError: result.exitCode != 0)
    }
  }
#endif
...
func resolvePath(_ path: String, context: ToolExecutionContext, forWrite: Bool) throws -> URL {
  ...
  let raw = URL(fileURLWithPath: path, relativeTo: path.hasPrefix("/") ? nil : base)
    .standardizedFileURL
  let allowedRoots =
    forWrite
...
extension URL {
  fileprivate func isContained(in root: URL) -> Bool {
...
func relativePath(_ child: URL, base: URL) -> String {
  ...
  public enum ProcessRunner {
    public static func run(
      _ executable: String, arguments: [String], currentDirectory: URL?, stdin: String?,
      timeout: TimeInterval
    ) async throws -> ProcessResult {
      guard timeout.isFinite else {
        throw CodexCoreError.invalidInput("Process timeout must be finite")
      }
      let timeout = max(timeout, 0.1)
      let roundedNanoseconds = (timeout * 1_000_000_000).rounded(.towardZero)
      guard let timeoutNanoseconds = UInt64(exactly: roundedNanoseconds) else {
        throw CodexCoreError.invalidInput("Process timeout is outside the supported range")
      }
  ...
  private func patchStripCount(_ value: JSONValue?) throws -> Int {
    guard let value else { return 0 }
    guard let number = value.doubleValue,
      number.isFinite,
      number >= 0,
      let strip = Int(exactly: number)
    else {
      throw CodexCoreError.invalidInput("Patch strip count must be a non-negative integer")
    }
    return strip
  }
#endif
