...
public enum AgentEvent: Sendable, Equatable, CustomStringConvertible {
  ...
  case codeModeNotification(CodeModeNotification)
  ...
  public var description: String {
    ...
    case .turnStarted(let threadID, let turnID):
      return "turnStarted(thread: \(threadID), turn: \(turnID))"
    case .turnCompleted(_, let turnID, let status, _):
      return "turnCompleted(turn: \(turnID), status: \(status.rawValue))"
    case .itemStarted(let item):
      return "itemStarted(\(item.kind.rawValue): \(item.summary ?? item.id))"
    case .itemDelta(let itemID, let delta): return "itemDelta(\(itemID): \(delta))"
    case .itemCompleted(let item):
      return "itemCompleted(\(item.kind.rawValue): \(item.summary ?? item.id))"
    ...
    case .toolCompleted(let call, let result):
      return "toolCompleted(\(call.name): \(result.summary))"
    case .codeModeNotification(let notification):
      return "codeModeNotification(\(notification.callID): \(notification.text))"
