...
final class OpenAIResponsesTransportTests: XCTestCase {
  ...
  func testDataRequestUsesBoundedExponentialBackoffAndEmitsTerminalFailure() async throws {
    ...
    do {
      _ = try await client.createBackgroundResponse(
        ResponsesRequest(
          model: "gpt-5.6-sol",
  ...
  func testAuthenticationRefreshRetainsIdentityWithoutConsumingRetryBudget() async throws {
    ...
    let snapshot = try await client.createBackgroundResponse(
      ResponsesRequest(
        model: "gpt-5.6-sol",
  ...
  func testCancellationDuringBackoffPropagatesWithoutAnotherAttempt() async throws {
    ...
    let task = Task {
      try await client.createBackgroundResponse(
        ResponsesRequest(
          model: "gpt-5.6-sol",
  ...
  func testDataConnectionFailureRetriesWithSameRequestIdentity() async throws {
    let attempts = TransportLocked(0)
    let requests = TransportLocked<[TransportCapturedRequest]>([])
    ...
    TransportStubURLProtocol.handler = { request in
      ...
      return TransportStubURLProtocol.response(
        for: request,
        body: #"{"id":"resp_recovered","status":"queued","background":true}"#
      )
    }
    ...
    let client = OpenAIResponsesClient(
      ...
      options: .init(endpoint: URL(string: "https://example.test/v1/responses")!),
      session: transportTestSession(),
      transportPolicy: .init(maximumRetryCount: 1, initialBackoff: 0),
      sleeper: { _ in }
    )
    ...
    let snapshot = try await client.createBackgroundResponse(
      ResponsesRequest(
        model: "gpt-5.6-sol",
        input: [ResponseInputBuilder.userMessage("retry transport")]
      ))

    XCTAssertEqual(snapshot.id, "resp_recovered")
    XCTAssertEqual(attempts.value, 2)
    XCTAssertEqual(requests.value[0].requestID, requests.value[1].requestID)
    XCTAssertEqual(requests.value[0].idempotencyKey, requests.value[1].idempotencyKey)
  }

  func testInterruptedStreamRetriesBeforeSemanticOutputWithSameIdentity() async throws {
    let attempts = TransportLocked(0)
    let requests = TransportLocked<[TransportCapturedRequest]>([])
    ...
    for try await event in client.streamResponse(
      ResponsesRequest(
        model: "gpt-5.6-sol",
        input: [ResponseInputBuilder.userMessage("retry stream")]
      ))
    {
      events.append(event)
    }

    XCTAssertTrue(events.contains(.completed(responseID: "resp_done", usage: nil)))
    XCTAssertEqual(attempts.value, 2)
    XCTAssertEqual(requests.value[0].requestID, requests.value[1].requestID)
    XCTAssertEqual(requests.value[0].idempotencyKey, requests.value[1].idempotencyKey)
  }

  func testInterruptedStreamDoesNotRetryAfterSemanticOutput() async throws {
    let attempts = TransportLocked(0)
    TransportStubURLProtocol.handler = { request in
      attempts.withValue { $0 += 1 }
      return TransportStubURLProtocol.response(
    ...
    do {
      for try await event in client.streamResponse(
        ResponsesRequest(
          model: "gpt-5.6-sol",
          input: [ResponseInputBuilder.userMessage("do not replay")]
        ))
      {
        events.append(event)
      }
      ...
      XCTAssertTrue(String(describing: error).contains("before response.completed"))
    }

    XCTAssertEqual(events, [.outputTextDelta("partial")])
    XCTAssertEqual(attempts.value, 1)
  }
}
