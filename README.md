...
Nested tools also emit ordinary `AgentEvent.toolStarted` and `toolCompleted` events with their actual registry names, arguments, and full `ToolResult`, including errors and media blocks. The matching call IDs are unique per cell/invocation; `ToolCall.caller` records `type: "program"`, `caller_id` for the outer `exec`, and `cell_id`. Apps can display these actual tool activities while treating `exec`/`wait` as execution details. Nested events do not insert duplicate calls or outputs into model replay. Custom code-mode engines should invoke `CodeModeExecutionRequest.runNestedTool` to preserve this lifecycle and the shared approval/sandbox path.
